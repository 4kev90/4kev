...
$currentPage =  preg_replace("/[^0-9]/","", basename($_SERVER['PHP_SELF'])); 
$aa = "SELECT * FROM posts WHERE ID = $currentPage";
...
/*
// ALLOW ONLY LOGGED IN USERS TO POST IMAGES
if($image && !$loggedIn) {
...
$x = "SELECT COUNT(*) AS replies FROM posts WHERE replyTo = $currentPage";
...
$uploadOk = 1;
    ...
    $sql = "INSERT INTO posts (name, options, commento, dateTime, replyTo, ipAddress, imageUrl, board, image, loggedIn) VALUES ('$name', '$options', '$comm', '$date', $currentPage, '$ipAddr', '$url', '$boardName', '$newName', '$loggedIn')";
    mysqli_query($con, $sql);

    //bump thread
    if(($options != "sage") && ($options != "SAGE")) {
        $selectSQL = "SELECT ID FROM posts ORDER BY ID DESC LIMIT 1;";
        $result = (mysqli_query($con, $selectSQL) );
        while($row = mysqli_fetch_assoc( $result )) 
            $newBump = $row['ID'];
        $updateSQL = "UPDATE posts SET bump=$newBump WHERE ID=$currentPage;";
...
<?php
    $err = $_GET['err'];
    if($err == 1)
        echo '<table><td style="height:30px; background:lightcoral; border:1px solid darkred; border-spacing: 1px; padding: 3px;"><p>You must be logged in to upload images</p></td></table>';
?>
...
<!--submission form-->
<!--il cancelletto significa che la form � submitted alla same page-->
...
while( $row = mysqli_fetch_assoc( $selectRes ) ){
    ...
    $rowImageUrl = str_replace("style","nope", $rowImageUrl);  //protection against xss attack
    $idUrl = $rowID . 'a';

    $space = str_repeat('&nbsp;', 2);  //spaces between picture and text
     
    if($row['ID'] == $currentPage || $row['replyTo'] == $currentPage) {
        ...
        if($row['imageUrl'])
        echo "<td style='vertical-align:top'><img class='smallpic' id='{$idUrl}' src='$rowImageUrl'></td>";
